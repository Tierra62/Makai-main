...
function SurveyManager(props) {
  ...
  useEffect(() => {
    surveyBuilderService
      .getSurveysByCreator(
        activeSurveys.pageIndex,
        activeSurveys.pageSize,
        currentUserId
      )
  ...
  const onGetAllSurveysError = (err) => {
    _logger({ error: err });
    toast.error("survey retrieve failure.");
  };
  ...
  return (
    <>
      <Card className="text-center">
        <h1 className="mt-4">Surveys Manager</h1>
        <Section
              ...
              <Row>
                <Container name="surveysContainer" className="col">
                  <Card className="m-2 rounded-0">
                    <div>
                      <h4 className="text-center m-4">All the Surveys</h4>
                    </div>
